...
1. **Wrangler routes (already in repo):** `wrangler.toml` includes a route for `go.suqram.com/*` on zone `suqram.com`. After deploy, ensure the zone is active in your Cloudflare account so the route applies.
...
Set these via **Cloudflare Secrets** (not in `wrangler.toml`):
...
2. **D1:** Create DB, apply schema, set `database_id` in `wrangler.toml`:
   ```bash
   wrangler d1 create eig-db
   wrangler d1 execute eig-db --file=sql/schema.sql --remote
   ```
...
Set in **wrangler.toml** `[vars]` or **Secrets** (production):

```toml
# wrangler.toml [vars] – example
...
3. **D1:** Apply schema (includes `live_tests` and `live_test_events`):
   ```bash
   wrangler d1 execute EIG_DB --file=sql/schema.sql --remote
   ```
